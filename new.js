...
window.onload = function () {
  ...
  function render (content, contents, pathName, formHandler) {
    ...
    if (/^\/posts\/\d+$/.test(pathName)) {
      const groups = /\/posts\/(\d+)/.exec(pathName)
      handlePost()
...
function handlePost (e) {
  ...
  let groups = /\/posts\/(\d+)/g.exec(pathName)

  console.log(groups)
  fetch(`http://localhost:3000/posts/${groups[1]}`)
    .then(function (response) {
      return response.json()
    })
    .then(function (data) {
      let postTemp = `<div class="post-preview" id= "${data.id}">
        ...
        <div class="comment" style="margin: 20px 20px 20px 20px;"> </div>
      ...
      fetch(`http://localhost:3000/comments`)
        ...
        .then(function (allCom) {
          ...
          innerCom = document.querySelector('.comment')
          console.log(innerCom)
          filterdCom.forEach(singleFCom => {
            innerCom.innerHTML += `<div style="display: block;
                                                    width: 100%;
...
function commentsHandler (e) {
  ...
  document.querySelector('.comment').innerHTML += `<div style="display: block;
                                                    width: 100%;
                                                    height: calc(1.5em + .75rem + 2px);
                                                    padding: .375rem .75rem;
                                                    margin-bottom:10px;
